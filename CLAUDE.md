...
- **Color:** warm beige/tan (`--purple` family — name kept from the lavender era, hex is now beige) is the brand and primary action color. Green = growth/gains. Gold = gamification, **Learn only**. Red = loss/caution, used sparingly, never as an alarm.
- **Feature colors** (which money area a screen belongs to — separate from gain/loss and from Learn's gold): **Investing = purple** (`--invest` family — the *original* lavender-purple, kept as its own dedicated color, NOT the beige brand tokens), **Saving = yellow** (`--save`), **Spending = orange** (`--spend`), **Loan = a deeper burnt-orange** (`--loan`, same family as Spending but distinguishable). Never repurpose green/red for this. Full table in [`docs/FISCALLY_BRAND.md`](docs/FISCALLY_BRAND.md#4-color-system-tokens).
